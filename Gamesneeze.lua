...
do
  ...
  function library:UpdateColor(ColorType, ColorValue)
    ...
    function library:Loader(info)
        ...
        theme.accent = accent
        --
        local window = {pages = {}, loader = true, isVisible = false, pageammount = pageammount, callback = callback, wminfo = "Aurora ", currentPage = nil, fading = false, dragging = false, drag = Vector2.new(0,0), currentContent = {frame = nil, dropdown = nil, multibox = nil, colorpicker = nil, keybind = nil, textbox = nil}}
        --
    ...
    function library:New(info)
        ...
        theme.accent = accent
        --
        local window = {pages = {}, loader = style == 2, init = false, pageammount = pageammount, isVisible = false, callback = callback, uibind = Enum.KeyCode.Z, wminfo = "Aurora ", currentPage = nil, fading = false, dragging = false, drag = Vector2.new(0,0), currentContent = {frame = nil, dropdown = nil, multibox = nil, colorpicker = nil, keybind = nil, textbox = nil}}
        --
        local main_frame = utility:Create("Frame", {Vector2.new(0,0)}, {
            Size = utility:Size(0, size.X, 0, size.Y),
            Position = utility:Position(0.5, -(size.X/2) ,0.5, -(size.Y/2)),
    ...
    function library:Page(info)
        ...
        --
        return setmetatable(page, pages), page
    end
    --
    ...
    --
        function pages:PlayerList(info)
            local info = info or {}
			 local callback = info.callback or info.Cllback or function() end             
                                ...
                                for i,v in pairs(button.holder.buttons) do
                                    ...
                                    if utility:MouseOverDrawing({v[2].Position.X, v[2].Position.Y, v[2].Position.X + v[2].Size.X, v[2].Position.Y + v[2].Size.Y}) and value ~= button.current then
                                        ...
                                        if button.selection then
											...
											callback(button.selection[1], button.selection[3])
	   ...
	   function pages:BotList(info)
            local info = info or {}
	    local accounts = info.accounts or info.Accounts or info.Alts or info.alts or {}
	    local callback = info.callback or info.Callback or function() end             
            ...
            --
            utility:Connection(plrs.PlayerAdded, function(Player)
		if Value ~= game.Players.LocalPlayer and table.find(accounts, Value.UserId) then
                	playerList.players[#playerList.players + 1] = {Player, Player.Name, "None", false}
			playerList:UpdateScroll()
		end
                --
    ...
    function sections:Label(info)
		...
		local localname = info.localname or info.variablename or ""
        ...
        if localname ~= nil then
            library.miscs[localname] = label_title
        end
...
--
return library, utility, library.pointers, theme, library.miscs
